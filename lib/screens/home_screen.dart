...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _startMonitoring() {
    _monitorTimer = Timer.periodic(const Duration(seconds: 5), (timer) async {
      ...
      for (var s in stats) {
        if (foregroundApp == null || int.parse(s.lastTimeUsed!) > int.parse(foregroundApp.lastTimeUsed!)) {
          foregroundApp = s;
        }
      }

      if (foregroundApp != null && foregroundApp.packageName != 'com.example.games_parenting_control') {
        ...
        if (index != -1) {
          ...
          if (_remainingSeconds > 0) {
            ...
            StorageService.saveGamePlayed(_games[index].name, _games[index].totalPlayedSecondsToday);
          } else {
            // Time is up, block it
            _bringToForeground();
          }
  ...
  int get _remainingSeconds =>
      (_dailyLimitMinutes * 60 - _totalPlayedSecondsToday).clamp(0, _dailyLimitMinutes * 60);
  ...
  void _launchGame(GameEntry game) async {
    ...
    if (playedSecondsResult != null && playedSecondsResult > 0) {
      ...
      await StorageService.saveGamePlayed(game.name, game.totalPlayedSecondsToday);
    }
  }
  ...
  void _showLockedDialog() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Kamu sudah mencapai batas waktu hari ini. Istirahat dulu ya!'),
        actions: [
          TextButton(
